...
Die Architektur basiert im Frontend auf dem React JavaScript-Framework \cite{react}, im Backend auf dem Python-Framework Flask \cite{flask} und für die Persistenz wird die NoSQL Datenbank MongoDB \cite{mongodb} verwendet. 
Frontend, Backend und die MongoDB wurden mittels Docker \cite{docker} containerisiert, um eine hochskalierung der Webanwendung zu ermöglichen.
Als CSS Framework wurde Tailwind CSS \cite{noauthor_tailwind_nodate} verwendet und für die Datenaquise auf die Instagram-API \cite{InstaApi} zurückgegriffen.}
\end{abstract}
